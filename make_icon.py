...
def draw_icon(size: int) -> Image.Image:
    """Crop symbol from logo.png (dark background), resize to target size."""
    src = Image.open(LOGO_PATH).convert("RGBA")
    w, h = src.size

    # Crop to just the symbol — skip "DOCTOR" and subtitle text (top ~62%)
    symbol = src.crop((0, 0, w, int(h * 0.62)))

    # Resize directly — logo already has dark background, no processing needed
    symbol = symbol.resize((size, size), Image.LANCZOS)
